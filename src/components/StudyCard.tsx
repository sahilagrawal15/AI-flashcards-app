...
export function StudyCard({ card, onComplete, onSkip }: StudyCardProps) {
  ...
  return (
    <div className="max-w-xl mx-auto">
      <div 
        className={`bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 min-h-[250px] flex flex-col transition-all duration-300 transform ${
          isFlipped ? 'rotate-y-180' : ''
        }`}
        style={{ perspective: '1000px' }}
      >
        <div className="flex-1 flex items-center justify-center p-4">
          <div className={`w-full transition-opacity duration-300 ${isFlipped ? 'hidden' : 'block'}`}>
            <p className="text-lg text-gray-900 dark:text-white text-center">
              {card.front_text}
            </p>
          </div>
          <div className={`w-full transition-opacity duration-300 ${isFlipped ? 'block' : 'hidden'}`}>
            <p className="text-lg text-gray-900 dark:text-white text-center">
              {card.back_text}
            </p>
          </div>
